 ...
 * targets (loopback, link-local / cloud metadata, RFC1918 ranges). This is a
...
export function assertPublicHttpUrl(rawUrl: string): URL {
  ...
  if (blockedHosts.includes(host)) throw new Error('That URL is not allowed');
  ...
  if (ipv4) {
    ...
    if (isPrivate) throw new Error('That URL is not allowed');
  }
